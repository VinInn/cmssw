...
>
__device__  
void radixSortImpl(T const * __restrict__ a, uint16_t * ind, uint16_t * ind2, uint32_t size, RF reorder) {
...
>
__device__
void radixSort(T const * a, uint16_t * ind, uint16_t * ind2, uint32_t size) {
...
__device__
void radixSortMulti(T const * v, uint16_t * index, uint32_t const * offsets, uint16_t * workspace) {

  auto a = v+offsets[blockIdx.x];
  auto ind = index+offsets[blockIdx.x];
  auto ind2 = workspace+offsets[blockIdx.x];
