...
namespace magfieldparam {
  ...
  // in meters and T  (Br needs to be multiplied by r)
    void compute(T r2, T z, T& Br, T& Bz) const {
      ...
      // vectorization: less instruction, slower!
#ifdef BCYCL_VECT
      constexpr int VS = 4; // 4, it will not vectorize 2!
      T vv[VS] ={u,v};
      T fg[VS][4];
      T * fu = fg[0];
      T * gv = fg[1];
      T zoff[VS] = {z-pars.prm[7],z+pars.prm[7]};
      T gpar[VS];
      for (int i=0; i<VS; ++i) {
         ffunkti(vv[i],fg[i]);
      ...
      ffunkti(u,fu);
      ffunkti(v,gv);
      T g2 = myExp(-(z-pars.prm[7])*(z-pars.prm[7])*pars.coeff) +
      ...
      T corBz=-pars.prm[6]*g2; // double Gaussian
      Br+=corBr;
      Bz+=corBz;
    }
